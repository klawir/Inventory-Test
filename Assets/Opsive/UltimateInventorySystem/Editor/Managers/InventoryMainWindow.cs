    ...
    public class InventoryMainWindow : MainManagerWindow
    {
        ...
        private void NavigateToInternal(object obj)
        {
            if (obj is Core.ItemCategory itemCategory) {
                var manager = Open(typeof(ItemCategoryManager));
                (manager as ItemCategoryManager).Select(itemCategory);
            }
            if (obj is Core.ItemDefinition itemDefinition) {
                var manager = Open(typeof(ItemDefinitionManager));
                (manager as ItemDefinitionManager).Select(itemDefinition);
            }
            if (obj is Crafting.CraftingCategory craftingCategory) {
                var manager = Open(typeof(CraftingCategoryManager));
                (manager as CraftingCategoryManager).Select(craftingCategory);
            }
            if (obj is Crafting.CraftingRecipe craftingRecipe) {
                var manager = Open(typeof(CraftingRecipeManager));
                (manager as CraftingRecipeManager).Select(craftingRecipe);
            }
            if (obj is Exchange.Currency currency) {
                var manager = Open(typeof(CurrencyManager));
                (manager as CurrencyManager).Select(currency);
            }
        }
