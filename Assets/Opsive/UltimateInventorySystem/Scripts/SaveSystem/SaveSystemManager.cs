...
namespace Opsive.UltimateInventorySystem.SaveSystem
{
    ...
    public class SaveSystemManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Open the save folder path.
        /// </summary>
        [ContextMenu("OpenSaveFolderPath")]
        public void OpenSaveFolderPath()
        {
#if UNITY_EDITOR
            UnityEditor.EditorUtility.OpenFilePanel("Save file", GetSaveFolderPath(), "");
#else
            Debug.LogError("Opening the save folder path can only be done in the editor.");
#endif
        }

        /// <summary>
        /// Write the readable registered objects names and IDs in the manager to disk.
        /// </summary>
        /// <param name="log">Should it also be logged in the console?</param>
        public void WriteReadableRegisteredObjectsNameAndIDsToDisk(bool log)
        {
            var readableNamesAndIDs = InventorySystemManager.Instance.GetReadableRegisteredObjectsNamesAndIDs();
            var file = $"{GetSaveFolderPath()}/RegisteredObjectsNameAndIDs.{m_SaveFileExtension}";
            ...
            using (FileStream fs = File.Create(file)) {
                ...
                WriteToFile(fs, readableNamesAndIDs);
            }

            if (log) {
                Debug.Log(readableNamesAndIDs);
            }
        }
        ...
        protected virtual void SaveToDiskInternal(int saveIndex)
        {
            ...
            m_LoadedSaveData = new SaveDataInfo(m_LoadedSaveData, saveIndex);
            ...
            CreateDebugSaveDataFile(m_Saves[saveIndex]);
        }
