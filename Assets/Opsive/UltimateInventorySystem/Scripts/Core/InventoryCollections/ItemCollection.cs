...
namespace Opsive.UltimateInventorySystem.Core.InventoryCollections
{
    ...
    public delegate void OnItemAddedEventHandler(ItemInfo originalItemInfo, ItemStack addedItemStack);
    public delegate void OnItemRemovedEventHandler(ItemInfo removedItemInfo);
    public delegate void OnItemAddOverflowEventHandler(ItemInfo originalItemInfo, ItemInfo itemInfoAdded, ItemInfo rejectedItemInfo);
    ...
    public class ItemCollection
    {
        ...
        public virtual bool HasItem(ItemDefinition itemDefinition, bool checkInherently, bool countStacks = false)
        {
            return HasItem(new ItemDefinitionAmount(itemDefinition, 1), checkInherently, countStacks);
        }

        /// <summary>
        /// This function checks if the ItemCollection contains an item with the itemDefinition.
        /// </summary>
        /// <param name="itemDefinitionAmount">The itemDefinition of the item to check.</param>
        /// <param name="checkInherently">Take into account the children of the item definition.</param>
        /// <param name="countStacks">If true count the stacks number not the stacks amounts.</param>
        /// <returns>Returns true if the amount of items with the itemDefinition in the collection is equal or bigger than the amount specified.</returns>
        ...
        /// </summary>
        /// <param name="category">The category of the items being checked.</param>
        ...
        public virtual bool HasItem(ItemCategory category, bool checkInherently, bool countStacks = false)
        {
            return HasItem(new ItemCategoryAmount(category, 1), checkInherently, countStacks);
        }
        ...
        protected virtual void NotifyAdd(ItemInfo itemInfo, ItemStack addedItemStack)
        {
            ...
            OnItemAdded?.Invoke(itemInfo, addedItemStack);
        ...
        /// <summary>
        /// Send events that an item was removed from the collection.
        /// </summary>
        /// <param name="removedItemInfo">The item info removed</param>
        protected virtual void NotifyItemRemoved(ItemInfo removedItemInfo)
        {
            if (m_Inventory != null) {
        ...
        public virtual ItemInfo? RemoveItemCondition(ItemInfo itemInfo)
        {
            ...
            if (itemInfo.Item.IsMutable && !itemInfo.Item.IsUnique) {
                ...
                var itemStackIsSimilar = itemInfo.ItemStack != null &&
                                         itemInfo.ItemStack.Item?.ItemDefinition == itemInfo.Item.ItemDefinition;

                if (itemStackIsSimilar == false) {
                    var similarItemInfo = GetItemInfo(itemInfo.Item);
                    if (similarItemInfo.HasValue) {
                        itemInfo = (itemInfo.Amount, similarItemInfo.Value);
                    }
        ...
        {
            return string.Format("ItemCollection({1}) {0}", m_Name, m_Purpose);
        }
