...
namespace Opsive.UltimateInventorySystem.Core.InventoryCollections
{
    ...
    public class ItemCollectionGroup
    {
        ...
        public event OnItemAddOverflowEventHandler OnItemAddOverflow;

        protected Action m_OnItemCollectionUpdate;
        protected virtual void HandleOnItemCollectionUpdate()
        {
            OnItemCollectionUpdate?.Invoke();
        }
        
        protected OnItemAddedEventHandler m_OnItemAdded;
        protected virtual void HandleOnItemAdded(ItemInfo originalItemInfo, ItemStack addedItemStack)
        {
            OnItemAdded?.Invoke(originalItemInfo, addedItemStack);
        }
        
        protected  OnItemRemovedEventHandler m_OnItemRemoved;
        protected virtual void HandleOnItemRemoved(ItemInfo removedItemInfo)
        {
            OnItemRemoved?.Invoke(removedItemInfo);
        }
        
        protected OnItemAddOverflowEventHandler m_OnItemAddOverflow;
        protected virtual void HandleOnItemAddOverflow(ItemInfo originalItemInfo, ItemInfo itemInfoAdded, ItemInfo rejectedItemInfo)
        {
        ...
        public virtual IReadOnlyList<ItemCollection> ItemCollections
        {
            ...
            set
            {
                if (m_ItemCollections != null) {
                    for (int i = 0; i < m_ItemCollections.Count; i++) {
                        ...
                        RegisterItemCollectionEvents(itemCollection, false);
                    }
                }
                ...
                m_ItemCollections.AddRange(value);
                ...
                if (m_ItemCollections != null) {
                    for (int i = 0; i < m_ItemCollections.Count; i++) {
                        ...
                        RegisterItemCollectionEvents(itemCollection, true);
                    }
                }
            }
        }

        protected virtual void RegisterItemCollectionEvents(ItemCollection itemCollection, bool register)
        {
            //Cache the event handlers.
            if (m_EventsInitialized == false) {
                m_OnItemCollectionUpdate = HandleOnItemCollectionUpdate;
                m_OnItemAdded = HandleOnItemAdded;
                m_OnItemRemoved = HandleOnItemRemoved;
                m_OnItemAddOverflow = HandleOnItemAddOverflow;
                m_EventsInitialized = true;
            }

            if (register) {
                //Register the events.
                itemCollection.OnItemCollectionUpdate += m_OnItemCollectionUpdate;
                itemCollection.OnItemAdded += m_OnItemAdded;
                itemCollection.OnItemRemoved += m_OnItemRemoved;
                itemCollection.OnItemAddOverflow += m_OnItemAddOverflow;
            } else {
                //Register the events.
                itemCollection.OnItemCollectionUpdate -= m_OnItemCollectionUpdate;
                itemCollection.OnItemAdded -= m_OnItemAdded;
        ...
        /// <summary>
        /// Set the ItemCollections by name.
        /// </summary>
        /// <param name="inventory">The Inventory where the item collection are located.</param>
        /// <param name="itemCollectionNames">THe item collection names subset.</param>
        /// <param name="allCollectionsIfArrayIsEmpty">If the item collection name list is empty, add all collection in the Inventory</param>
        /// <param name="logWarningIfMissing">Log a warning if one of the collection names does not exist in the Inventory?</param>
        public virtual void SetItemCollections(Inventory inventory, IReadOnlyList<string> itemCollectionNames, bool allCollectionsIfArrayIsEmpty, bool logWarningIfMissing)
        {
            if (itemCollectionNames == null || itemCollectionNames.Count == 0) {
                if (allCollectionsIfArrayIsEmpty) {
                    ItemCollections = inventory.ItemCollections;
                } else {
        ...
        /// <summary>
        /// Add an item collection to the list of item Collections in the group.
        /// </summary>
        /// <param name="itemCollection">The itemCollection to add to the group.</param>
        public virtual void AddItemCollection(ItemCollection itemCollection)
        {
            if (itemCollection == null || m_ItemCollections.Contains(itemCollection)) {
                return;
            }
        ...
        /// <summary>
        /// Remove an item collection to the list of item Collections in the group.
        /// </summary>
        /// <param name="itemCollection">The itemCollection to remove from the group.</param>
        public virtual void RemoveItemCollection(ItemCollection itemCollection)
        {
            if (itemCollection == null || m_ItemCollections.Contains(itemCollection) == false) {
                return;
            }
