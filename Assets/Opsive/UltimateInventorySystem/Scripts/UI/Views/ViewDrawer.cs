...
namespace Opsive.UltimateInventorySystem.UI.Views
{
    ...
    public abstract class ViewDrawer<T> : ViewDrawerBase
    {
        ...
        /// <param name="disableAndHideEmptiesIndex">Disable and hide the slots for elements that are over the index (-1 do not disable).</param>
        public virtual void DrawViews(int startIndex, int endIndex, IReadOnlyList<T> elements, int disableAndHideEmptiesIndex)
        {
            if (startIndex < 0) {
            ...
            for (int i = startIndex; i < endIndex; i++) {
                ...
                if (i < elementsEnd) {
                    ActivateViewSlot(viewIndex, true);
                    DrawView(viewIndex, i, elements[i]);
                } else {
                    if (!m_UseViewSlot || disableAndHideEmptiesIndex <= -1 || i - disableAndHideEmptiesIndex < 0) {
                        ActivateViewSlot(viewIndex, true);
                        DrawView(viewIndex, i, default);
                    } else {
                        ActivateViewSlot(viewIndex, false);
                    }
                }
            }
        }
        ...
        protected virtual View<T> InstantiateViewWithSlot(GameObject prefab, int boxIndex, int elementIndex, T element)
        {
            ...
            var viewSlot = m_ViewSlots[boxIndex];
            
            if (viewSlot.isActiveAndEnabled == false) {
                viewSlot.gameObject.SetActive(true);
            }
            var view = ObjectPool.Instantiate(prefab, viewSlot.transform).GetComponent<View<T>>();
